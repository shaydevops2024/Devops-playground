...
const TerraformPlayground = () => {
  ...
  useEffect(() => {
    checkTerraformPrerequisites();
    setupWebSocket();

    return () => {
      wsClient.removeListener('terraform-playground');
    };
  }, []);
  ...
  const setupWebSocket = () => {
    ...
    wsClient.addListener('terraform-playground', (message) => {
      if (message.type === 'log') {
        setLogs((prev) => [
          ...prev,
          {
  ...
  const checkTerraformPrerequisites = async () => {
    try {
      const result = await checkPrerequisites('terraform');
      setPrerequisites(result);
      
      if (result.ready) {
        const scenariosList = await listScenarios('terraform');
  ...
  const handleExecuteScenario = async () => {
    ...
    try {
      await executeScenario('terraform', selectedScenario);
    } catch (error) {
      setExecuting(false);
      setLogs((prev) => [
        ...prev,
    ...
    return (
      <div>
        ...
        <div className="container">
          <LoadingSpinner message="Checking Terraform prerequisites..." />
        </div>
      </div>
    );
  }
      ...
      <div>
        ...
        <div className="container">
          <div className="card" style={{ maxWidth: '600px', margin: '40px auto' }}>
            ...
            <p>Terraform is not installed or not accessible on this server.</p>
            <p style={{ marginTop: '16px', color: 'var(--text-secondary)' }}>
              Please install Terraform using:
            </p>
            <pre
              style={{
            ...
            >
              {prerequisites?.command || 'terraform version'}
            </pre>
          </div>
        </div>
      </div>
